...
public abstract class AndroidEncoder {
    ...
    public void adjustBitrate(int targetBitrate) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && mEncoder != null) {
    ...
    public void drainEncoder(boolean endOfStream) {
        if (endOfStream/* && VERBOSE*/) {
            if (isSurfaceInputEncoder()) {
                Log.d(TAG, "Stop: final video drain");
            } else {
                Log.d(TAG, "Stop: final audio drain");
            }
        }
        synchronized (mMuxer) {
            ...
            if (endOfStream) {
                if (VERBOSE) Log.d(TAG, "Stop: sending EOS to encoder for track " + mTrackIndex);
...
//                }
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (!endOfStream) {
                        ...
                        if (mEosSpinCount > MAX_EOS_SPINS) {
                            if (VERBOSE) Log.w(TAG, "Stop: Force shutting down Muxer. Encoder: "
                                    + mEncoder.getName());
                            mMuxer.forceStop();
                            break;
                        }
                        if (VERBOSE) Log.d(TAG, "Stop: no output available, spinning to await EOS");
                    }
                    ...
                    Log.d("AndroidEncoder", "added track to muxer" + newFormat);
                    ...
                    if (mBufferInfo.size >= 0) {    // Allow zero length buffer for purpose of sending 0 size video EOS Flag
                        ...
                        if (mForceEos) {
                            ...
                            Log.w(TAG, "Stop: Forcing EOS " + this.mEncoder.getName());
                        }
                    ...
                    if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                        if (!endOfStream) {
                            ...
                            if (VERBOSE)
                                Log.d(TAG, "Stop: end of stream reached for track " + mTrackIndex
                                        + "encoder: " + this.mEncoder.getName());
                        }
                        break;      // out of while
                    }
                }
            ...
            if (endOfStream && VERBOSE) {
                if (isSurfaceInputEncoder()) {
                    Log.i(TAG, "Stop: final video drain complete");
                } else {
                    Log.i(TAG, "Stop: final audio drain complete");
                }
            }
        }
    }
